...
function ContactList() {
  ...
  // Check if window is small or big
  useEffect(() => {
    setShowDetail(window.innerWidth >= 768)
    window.addEventListener('resize',() => setShowDetail(window.innerWidth >= 768))
  },[])
  ...
  useEffect(()=>{
    if(data){
        ...
        if(!edittedSelection){
            ...
            const newSelectedContactIndex = data.contact.findIndex(contactObj => contactObj.id === edittedSelection)
  ...
  return (
    <>
        {addMode || editMode ? <AddContact contactList={contactList} setEdittedSelection={setEdittedSelection} handleAddMode={handleAddMode} editMode={editMode} handleEditMode={handleEditMode} selectedContact={editMode ? selectedContact : {}}/> :
            <div className='row'>
                <> {showList ? 
