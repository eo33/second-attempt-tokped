...
function AddContact({
  ...
  useEffect(() => {
    const listOfNames = contactList.map(
      (contact) => contact.first_name + contact.last_name
    );
    let currentName = editMode
      ? edittedFirstName + edittedLastName
      : contactName.first_name + contactName.last_name;

    if (editMode) {
      // drop the current name from the listOfNames
      const currentNameIndex = listOfNames.indexOf(
  ...
  // Handle adding a new contact name with validation
